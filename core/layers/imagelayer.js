...
proto.getWMSLayerName = function({type='map'}={}) {
  ...
  if (legendMapBoolean && this.config.source && this.config.source.external && (this.config.source.layers || this.config.source.layer)) {
...
proto.getWmsUrl = function({type='map'}={}) {
  const legendMapBoolean = type === 'map' ? this.isExternalWMS() && this.isLayerProjectionASMapProjection() : true;
  return (legendMapBoolean && this.config.source && this.config.source.external && this.config.source.type === 'wms' && this.config.source.url) ?
